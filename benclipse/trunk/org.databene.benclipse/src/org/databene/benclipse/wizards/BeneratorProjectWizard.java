...
public class BeneratorProjectWizard extends Wizard implements INewWizard, Observer {
	...
	public boolean canFinish() {
		if (isUsingDB() && !targetDatabasePage.isPageComplete())
			return false;
	...
	public boolean performFinish() {
		...
		try {
			BenclipseProjectBuilder.runProjectBuilder(
					nameGroup.getProjectName(), 
					archetypePage.getSelectedArchetype(), 
					getJREGroup().getSelectedJRE(),
					dbInfo,
					generationConfig, 
					EclipseUtil.getActiveWorkbenchWindow());
			result = true;
		} catch (Exception e) {
			result = false;
		}
	...
	void updateControls() {
		// enablement of advanced controls
        boolean simple = isSimpleProject();
		getJREGroup().setEnabled(!simple);
		// enablement of advanced pages
